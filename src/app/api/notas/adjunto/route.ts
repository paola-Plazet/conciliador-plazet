...
export async function POST(req: NextRequest) {
  ...
  if (!body.noteId || !body.data || (!esImagen && !esPdf)) {
    return NextResponse.json({ error: "Solo se aceptan imágenes o PDF (noteId, mime, data)." }, { status: 400 });
  }
  const bytes = Buffer.from(body.data, "base64");
  if (bytes.length === 0 || bytes.length > MAX_BYTES) {
    return NextResponse.json({ error: `${esPdf ? "El PDF" : "La imagen"} está vacío o pesa más de 3 MB.` }, { status: 413 });
  }
  ...
  const adj = await prisma.noteAttachment.create({
    data: { noteId: nota.id, name: (body.name || (esPdf ? "comprobante.pdf" : "comprobante.jpg")).slice(0, 120), mime, size: bytes.length, data: bytes },
