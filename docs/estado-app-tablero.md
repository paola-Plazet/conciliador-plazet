...
- 24-sep (2): vínculo MANUAL factura↔cobro MP (`PrincipalVinculo`, `/api/principal/vincular` EDITOR+, pase 0 del cruce; botón "Sí, es este cobro" en las sugerencias y "deshacer"; script `scripts/principal-vincular.ts <fecha> <valor> "<nota>" <recibos...>`). Primer caso (Paola): ML 4-sep $117.000 = recibos 9062+9063 (FE P296+P297, Isable Orozco) — se olvidó un té en la primera; dif $3.350 por precio distinto. Recibo Karrot ≠ número FE: el reporte CUSTOMER_INVOICES da el mapeo (E-invoice prefix P + consecutive ↔ Sales order receipt).
- 24-sep (3): Mercado Pago por API ahora IGNORA las COMPRAS (pagos cuyo collector_id no es HABBIE 3438732386: Skydropx, cargos ML, compras en ML, tenis 5-sep…) — pedido de Paola. 456→448 operaciones; afecta también /resumen y /web (antes contaban como ingreso MP).
  Regla Paola para EGRESOS (futuro): Skydropx = gasto de Habbie (pagado con saldo MP); las compras en Mercado Libre se pagan con tarjeta PERSONAL → no son gasto de Habbie.
- 24-sep (4): Paola: los pedidos web PLAZET se facturan SOLOS por la integración Shopify→Karrot, en la tienda de donde sale la mercancía (no en Principal). Verificado: en sep Karrot solo tiene canal "POS - Karrot" (3.724 órdenes); la última venta que entró por la integración es la 3687 "SHOPIFY · Pago Online" 3-ago = #Plazet1798. Desde #Plazet1799 (21-ago) ninguno llegó → integración caída. La página marca esos cobros "no llegó de Shopify a Karrot".
- 24-sep (5): CORRECCIÓN: la web Plazet SÍ entra sola a Karrot, ubicación SHOPIFY (id 6320986b-…), canal SHOPIFY, usuario Federico, pero SIN método de pago → el reporte ALL_SALES_DETAIL_PAYMENT_METHOD no la trae. Nuevo: parser `karrot-shopify.ts` (reporte ALL_SALES por locationID), `lib/karrot-shopify-carga.ts` (source "karrot_shopify", bodega "SHOPIFY · Pago Online", excluido del deleteRange de ledger), `/api/cron/karrot-shopify`, `scripts/cargar-karrot-shopify.ts`, campo `Sale.fe` (factura electrónica). robot-bancos/diario_9am.py `cargar_shopify()` (últimos 10 días) tras cargar_karrot. /principal incluye SHOPIFY; aviso si entró sin factura electrónica. Sep: 9/9 pedidos Plazet con cobro; SIN FE: 9880, 10037, 10749, 10827, 11190 (y 6147, 6724 de ago).
